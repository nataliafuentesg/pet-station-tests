                    ...
                    <div key={c.id} className="mb-8 p-5 rounded-2xl" style={{ background: C.paper, border: `1px solid ${C.line}` }}>
                        ...
                        <div className="mb-4 grid gap-3">
                            {c.pdfUrl && (
                                <div>
                                    <div className="flex items-center justify-between mb-2">
                                        <span className="text-xs font-semibold" style={{ color: C.faint }}>DOCUMENTO ADJUNTO:</span>
                                    ...
                                    <div className="w-full rounded-xl overflow-hidden border border-slate-200" style={{ height: "320px", background: "#E2E7F1" }}>
                                        <iframe src={`${c.pdfUrl}#toolbar=0`} title={`Documento ${c.patient}`} className="w-full h-full border-none" />
                                    </div>
                                </div>
                            )}

                            {c.pdfs && c.pdfs.map((pdf, idx) => (
                                <div key={idx}>
                                    <div className="flex items-center justify-between mb-2">
                                        <span className="text-xs font-semibold" style={{ color: C.faint }}>ADJUNTO: {pdf.name.toUpperCase()}</span>
                                        <a href={pdf.url} target="_blank" rel="noopener noreferrer" className="text-xs font-semibold underline" style={{ color: C.blue }}>
                                            Abrir {pdf.name} ↗
                                        </a>
                                    </div>
                                    <div className="w-full rounded-xl overflow-hidden border border-slate-200 mb-2" style={{ height: "280px", background: "#E2E7F1" }}>
                                        <iframe src={`${pdf.url}#toolbar=0`} title={`${pdf.name} - ${c.patient}`} className="w-full h-full border-none" />
                                    </div>
                                </div>
                            ))}
                        </div>
