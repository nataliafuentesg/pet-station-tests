...
function Dot({ active, onClick }) {
  return <button onClick={onClick} className="rounded-full transition-all duration-150" style={{ width: active ? 22 : 14, height: active ? 22 : 14, background: active ? C.blue : "#fff", border: `2px solid ${active ? C.blue : C.faint}`, boxShadow: active ? `0 2px 8px ${C.blue}55` : "none", cursor: "pointer" }} />;
}

export default function App() {
  useFonts();
  const [phase, setPhase] = useState("loading"); // loading|invalid|already|intro|instructions|quiz|done
  ...
  useEffect(() => {
    ...
    jsonp(`${WEBAPP_URL}?action=check&token=${encodeURIComponent(tk)}&tipo=estilo`)
      .then((r) => { if (!r || !r.valid) { setReason(r ? r.reason : "error"); setPhase("invalid"); return; } setName(r.nombre || ""); setEmail(r.email || ""); setPhase("instructions"); })
  ...
  if (phase === "intro") return <Shell><div className="rounded-3xl p-8" style={card}>
    <h1 style={{ ...serif, fontSize: 36, lineHeight: 1.1, fontWeight: 500, color: C.navy }}>Evaluación de<br />estilo de trabajo</h1>
  ...
  if (phase === "instructions") return <Shell><div className="rounded-3xl p-8" style={card}>
    <h1 style={{ ...serif, fontSize: 32, lineHeight: 1.1, fontWeight: 500, color: C.navy }}>¿Cómo trabajas cuando estás en tu elemento?</h1>
    <p className="mt-4" style={{ color: C.sub, fontSize: 16, lineHeight: 1.6 }}>Son 15 preguntas. En cada una te ubicas entre dos formas de ser, según cómo te describirías normalmente en el trabajo.</p>
    ...
    <button onClick={begin} className="mt-6 w-full rounded-xl py-3.5 font-medium" style={{ background: C.blue, color: "#fff", fontSize: 16 }}>Comenzar</button>
  </div></Shell>;
  ...
  return <Shell>
    ...
    <div className="rounded-3xl p-7" style={card}>
      ...
      <div className="grid gap-7">
        {sec.items.map((it, i) => {
          ...
          return <div key={i} className="flex items-center justify-between gap-3">
    ...
    <div className="flex items-center justify-between mt-5">
      <button onClick={() => setSi((x) => Math.max(0, x - 1))} disabled={si === 0} className="rounded-xl px-5 py-2.5 text-sm font-medium" style={{ color: si === 0 ? C.faint : C.sub, border: `1px solid ${C.line}`, background: "#fff", cursor: si === 0 ? "default" : "pointer" }}>Atrás</button>
      {si < SECTIONS.length - 1
        ...
        : <button disabled={answered < TOTAL || sent === "sending"} onClick={finish} className="rounded-xl px-6 py-2.5 text-sm font-medium" style={{ background: answered >= TOTAL ? C.blue : C.line, color: answered >= TOTAL ? "#fff" : C.faint, cursor: answered >= TOTAL ? "pointer" : "not-allowed" }}>{sent === "sending" ? "Enviando…" : "Finalizar y enviar"}</button>}
    </div>
  </Shell>;
}
